...
class ReceiptPDFTests(TestCase):
    def setUp(self):
        ...
        self.order = ServiceOrder.objects.create(customer=customer, device=device, notes="ok")
    ...
    def test_payment_receipt_pdf_ok(self):
        payment = Payment.objects.create(order=self.order, device=self.order.devices.first(), amount=Decimal("50.00"), method="Efectivo")
